...
-include_lib("epic/include/epic.hrl").

run() ->
    run([key_press, key_release]).

run(EventMask) ->
    {ok,Win} = epx:window_create(50, 50, 100, 100, EventMask),
...
line_demo() ->
    {ok,Win} = epx:window_create(50, 50, 200, 100, []),
    epx:window_attach(Win),
    {ok,Pic} = epx:pixmap_create(200,100),
    epx:pixmap_attach(Pic),
    {ok,Gc} = epx:gc_create(),
